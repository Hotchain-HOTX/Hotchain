...
"""
burn-hotx: create a bitcoin burn address
By James C. Stroud
...
Call the program with a template burn address as the only argument::
    % burn-hotx LZBurnAddressForHotxPreminexxxxxxx
    LZBurnAddressForHotxPreminextStefG
For instructions, call the program with no arguments::
    % burn-hotx
    usage: burn-hotx TEMPLATE
...
class BurnhotxError(Exception):
    pass


class AlphabetError(BurnhotxError):
    pass
...
def usage():
    print "usage: burn-hotx TEMPLATE"
    print
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        usage()
    if sys.argv[1] == "test":
        template = "RXBurnAddressForHotxPreminexxxxxxx"
    else:
        template = sys.argv[1]
    for c in template:
        if c not in ABET:
