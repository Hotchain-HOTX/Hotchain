...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i hotchain-qt.dmg$ | xargs -i cp {} $OUTDIR/bin
    find $TRAVIS_BUILD_DIR -type f | grep -i hotchain-core.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    cp $TRAVIS_BUILD_DIR/src/qt/hotchain-qt $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/qt/hotchain-qt.exe $OUTDIR/bin/ || echo "no QT Wallet"
    cp $TRAVIS_BUILD_DIR/src/hotchaind $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/hotchain.exe $OUTDIR/bin/
    cp $TRAVIS_BUILD_DIR/src/hotchain-cli $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/hotchain-cli.exe $OUTDIR/bin/
