...
pkgdatadir = $(datadir)/hotchain
pkgincludedir = $(includedir)/hotchain
pkglibdir = $(libdir)/hotchain
pkglibexecdir = $(libexecdir)/hotchain
...
CONFIG_HEADER = $(top_builddir)/src/config/hotchain-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/hotchain-config.h.in \
...
ACLOCAL = ${SHELL} /root/Hotchain/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /root/Hotchain/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/Hotchain/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/Hotchain/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = hotchain-cli
BITCOIN_DAEMON_NAME = smartjuanad
BITCOIN_GUI_NAME = hotchain-qt
BITCOIN_TX_NAME = hotchain-tx
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/Hotchain/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = hotchain
PACKAGE_BUGREPORT = www.hotx.io
PACKAGE_NAME = Hotchain Wallet
PACKAGE_STRING = Hotchain Wallet 1.0.0
PACKAGE_TARNAME = hotchain
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /root/Hotchain
abs_srcdir = /root/Hotchain
abs_top_builddir = /root/Hotchain
abs_top_srcdir = /root/Hotchain
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/Hotchain/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_smartjuana_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_smartjuana_coverage.info test_smartjuana.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/hotchain-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/hotchain-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/hotchain-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/hotchain-config.h
$(top_srcdir)/src/config/hotchain-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/hotchain-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/hotchain-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/hotchain-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/hotchain-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_smartjuana.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_smartjuana -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_smartjuana_filtered.info: test_smartjuana.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_smartjuana_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_smartjuana_coverage.info: baseline_filtered_combined.info test_smartjuana_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_smartjuana_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_smartjuana_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_smartjuana_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_smartjuana.coverage/.dirstamp:  test_smartjuana_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_smartjuana.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_smartjuana.coverage/ total.coverage/ $(OSX_APP)
