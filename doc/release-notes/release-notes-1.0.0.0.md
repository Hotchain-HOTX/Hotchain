Hotchain Wallet version *1.0.0.0* is now available from:  <https://github.com/Hotchain-HOTX/Hotchain/releases>
...
Hotchain Wallet v1.0.0.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
### Deterministic zHOTX Seed Keys

zHOTX is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zHOTX outside of the wallet that the zHOTX had been minted on. zHOTX can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zHOTX minting

zHOTX minting now only requires 1 further mint (down from 2) to mature. zHOTX mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zHOTX Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zHOTX denomination has been spent or not.
...
* If zHOTX backup, auto generated name is `wallet-autozhotxbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
This allows running hotchaind without having to do any manual configuration.
...
Once a zHOTX has over 200 confirmations it becomes available to stake. Staking zHOTX will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zHOTX. So for example if a 1,000 zHOTX denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zHOTXs.
...
### Core Features
 - #582 `cb1632520` [Core] zHOTX v2: zPoS dzHOTX ezHOTX external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zHOTX balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zHOTX status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zHotx Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault hotchain-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zHOTX spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zHOTX. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zHOTX. (presstab)
